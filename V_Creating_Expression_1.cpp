...
bool helper(int idx,int n,int sum,int x,vector<int>&arr){
    if(idx == n ) return sum == x;

    bool add = helper(idx+1,n,sum+arr[idx],x,arr);
    bool sub = helper(idx+1,n,sum-arr[idx],x,arr);
...
void solve(){
    ...
    bool ans = helper(1,n,arr[0],x,arr);
